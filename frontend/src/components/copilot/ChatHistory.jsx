...
function ChatItem({ item, active, onClick, t }) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2">
        {pinned.length > 0 && (
          <CollapsibleSection title={t('Pinned')} defaultOpen={true}>
            {pinned.map((chat) => (<ChatItem key={chat.session_id} item={chat} active={activeChatId === chat.session_id} onClick={() => onSelectChat(chat.session_id)} t={t} />))}
          </CollapsibleSection>
        )}
        {todaySessions.length > 0 && (
          <CollapsibleSection title={t('Today')} defaultOpen={true}>
            {todaySessions.map((chat) => (<ChatItem key={chat.session_id} item={chat} active={activeChatId === chat.session_id} onClick={() => onSelectChat(chat.session_id)} t={t} />))}
          </CollapsibleSection>
        )}
        {olderSessions.length > 0 && (
          <CollapsibleSection title={t('Previous')} defaultOpen={true}>
