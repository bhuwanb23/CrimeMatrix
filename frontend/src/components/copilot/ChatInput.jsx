...
export default function ChatInput({ onSend, voiceEnabled: _voiceEnabled = false, onVoiceToggle: _onVoiceToggle, language = 'en', onLanguageChange }) {
  ...
  return (
    <div className="border-t border-[var(--border)] bg-[var(--bg-card)] px-6 py-3 flex-shrink-0">
      {/* Suggestion chips */}
      {showSuggestions && (
        <div className="flex gap-2 pb-3 justify-center flex-wrap">
          {suggestedQueries.map((q, i) => (
            <button
              ...
              className="px-4 py-1.5 rounded-full border border-[var(--border)] bg-[var(--bg-muted)] text-[var(--text-secondary)] text-xs hover:bg-blue-500 hover:text-white hover:border-blue-500 transition-all duration-150 whitespace-nowrap"
            >
              {q}
            </button>
          ))}
      ...
      {/* Input area */}
      <div className="flex items-center gap-2 bg-[var(--bg-input)] rounded-2xl px-4 py-2">
        <input
          type="text"
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-1 px-2 py-1 text-xs text-[var(--text-muted)] hover:text-[var(--text-secondary)] rounded-md hover:bg-[var(--bg-hover)] transition-colors"
          >
            {languages.find(l => l.code === language)?.label || 'EN'}
            <ChevronDown size={12} />
          </button>
          {langOpen && (
            <div className="absolute bottom-full right-0 mb-1 bg-[var(--bg-card)] border border-[var(--border)] rounded-lg shadow-lg py-1 z-50 min-w-[100px]">
              {languages.map((l) => (
                <button
                  key={l.code}
                  className={`w-full text-left px-3 py-1.5 text-xs hover:bg-[var(--bg-hover)] ${l.code === language ? 'text-blue-600 font-medium' : 'text-[var(--text-secondary)]'}`}
        ...
        <button className="p-1.5 text-[var(--text-muted)] hover:text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] rounded-md transition-colors" aria-label={t("Attach file")}>
          <Paperclip size={16} />
        </button>
