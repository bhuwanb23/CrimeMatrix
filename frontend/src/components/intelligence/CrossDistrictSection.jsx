    ...
    <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl overflow-hidden">
      ...
      <div className="px-5 py-4 border-b border-[var(--border)] flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold text-[var(--text-primary)]">Cross-District Intelligence</h3>
            <p className="text-[10px] text-[var(--text-muted)]">Statewide connection detection across districts</p>
          </div>
        </div>
      ...
      {matches.length === 0 ? (
        <div className="text-center py-12">
          <Globe size={32} className="mx-auto text-[var(--text-muted)] mb-3" />
          <p className="text-sm font-medium text-[var(--text-muted)] mb-1">No cross-district matches found</p>
          <p className="text-xs text-[var(--text-muted)]">Click "Detect Matches" to scan for statewide connections.</p>
        </div>
      ) : (
        <>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-[var(--border)]">
                  ...
                  <th className="px-5 py-3 text-[10px] font-semibold text-[var(--text-muted)] uppercase tracking-wider">Match Reason</th>
                  <th className="px-5 py-3 text-[10px] font-semibold text-[var(--text-muted)] uppercase tracking-wider">Districts</th>
              ...
              <tbody>
                {pageData.map((m, i) => {
                  ...
                  return (
                    <tr key={m.id || i} className="border-b border-[var(--border)] hover:bg-[var(--bg-hover)] transition-colors">
                      ...
                      <td className="px-5 py-3 text-xs font-medium text-[var(--text-primary)]">{m.match_reason}</td>
                      <td className="px-5 py-3">
                      ...
                      <td className="px-5 py-3 text-right">
                        <span className={`text-xs font-bold ${m.confidence >= 70 ? 'text-emerald-600' : m.confidence >= 40 ? 'text-amber-600' : 'text-[var(--text-muted)]'}`}>
                          {m.confidence}%
                        </span>
                      </td>
                      <td className="px-5 py-3">
