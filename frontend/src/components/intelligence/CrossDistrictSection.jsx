    ...
    <div className="bg-white border border-slate-200 rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-slate-900">{t('Cross-District Intelligence')}</h3>
          <span className="text-[10px] text-slate-400">{matches.length} {t('matches')}</span>
        </div>
        <button onClick={handleDetect} disabled={detecting}
          className="text-[10px] text-amber-500 hover:underline disabled:opacity-50">
          {detecting ? t('Detecting...') : t('Detect Matches')}
        </button>
      </div>
      ...
      {matches.length === 0 ? (
        <div className="py-6 text-center text-xs text-slate-400">
          ...
          <p>{t('No cross-district matches found')}</p>
          <p className="text-[10px] text-slate-300">{t('Click "Detect Matches" to scan for connections')}</p>
        </div>
      ) : (
                ...
                <div className="flex items-center gap-3 p-2 bg-slate-50 rounded-lg">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-medium text-slate-900">{t(m.match_reason)}</p>
                      <ExplainButton onClick={() => handleExplain(m.id)} loading={explainingId === m.id} />
                    </div>
                    <div className="flex items-center gap-2 text-[10px] text-slate-400">
                      <span>{t(m.district_1)}</span>
                      <ArrowRight size={10} />
                      <span>{t(m.district_2)}</span>
