...
export default function PredictionForecastChart({ forecast }) {
    ...
    return (
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl overflow-hidden min-h-[300px]">
        <div className="px-5 py-4 border-b border-[var(--border)] flex items-center gap-2.5">
          ...
          <h3 className="text-sm font-bold text-[var(--text-primary)]">{t('Crime Forecast')}</h3>
        </div>
    ...
    return (
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl overflow-hidden min-h-[300px]">
        ...
        <div className="flex flex-col items-center justify-center py-12">
          ...
          {historical.length > 0 && (
            <p className="mt-3 text-xs text-[var(--text-muted)]">Avg: <strong>{Math.round(historical.reduce((s, d) => s + (d.count || 0), 0) / historical.length)}</strong> crimes/day</p>
          )}
        </div>
      </div>
    )
  ...
  return (
    <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl overflow-hidden min-h-[300px]">
        ...
        <div className="flex items-end gap-[3px]" style={{ height: 160 }}>
          {historical.slice(-20).map((d, i) => (
            <div key={i} className="flex flex-col items-center flex-1 min-w-0">
              ...
              <span className="text-[7px] text-[var(--text-muted)] mt-1 truncate">{(d.date || '').slice(-5)}</span>
            </div>
          ))}
      ...
      {/* Info */}
      <div className="px-5 py-3 border-t border-[var(--border)] flex items-center gap-6 text-[10px] text-[var(--text-muted)]">
        <span>Confidence: <strong className="text-[var(--text-secondary)]">{forecast.confidence || 0}%</strong></span>
        <span>Data points: <strong className="text-[var(--text-secondary)]">{dataPoints}</strong></span>
        <span>Trend: <strong className="text-[var(--text-secondary)]">{forecast.trend || 'stable'}</strong></span>
      </div>
    </div>
  )
}
