    ...
    <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl overflow-hidden">
      ...
      <div className="px-5 py-4 border-b border-[var(--border)] flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold text-[var(--text-primary)]">Behavioral Profiling</h3>
            <p className="text-[10px] text-[var(--text-muted)]">Analyze criminal behavior patterns</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <input className="w-28 px-3 py-1.5 bg-[var(--bg-muted)] border border-[var(--border)] rounded-lg text-xs text-[var(--text-secondary)] focus:outline-none focus:border-amber-400"
      ...
      {stats && (
        <div className="px-5 py-3 border-b border-[var(--border)] flex items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            <span className="text-lg font-bold text-[var(--text-primary)]">{stats.criminals_profiled || 0}</span>
            <span className="text-[10px] text-[var(--text-muted)]">Criminals</span>
          </div>
          <div className="flex items-center gap-2">
      ...
      {!hasData ? (
        <div className="text-center py-12">
          ...
          <p className="text-sm font-medium text-[var(--text-muted)] mb-1">No behavioral profiles yet</p>
          <p className="text-xs text-[var(--text-muted)] mb-4">Enter a Criminal ID above and click Analyze to generate a behavioral profile.</p>
          <div className="inline-block bg-[var(--bg-muted)] rounded-lg px-4 py-3 text-[10px] text-[var(--text-muted)] text-left max-w-xs">
            <p className="m-0 font-medium text-[var(--text-secondary)] mb-1">How profiling works:</p>
          ...
          {riskAssessment.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-xs font-semibold text-[var(--text-secondary)]">Risk Assessment</h4>
              </div>
              <div className="space-y-2">
                {riskAssessment.map((r, i) => (
                  <div key={i} className="p-3 bg-[var(--bg-muted)] rounded-lg">
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-xs font-semibold text-[var(--text-primary)]">{r.alias}</span>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-mono text-[var(--text-muted)]">{r.risk_score}%</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {profiles.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-xs font-semibold text-[var(--text-secondary)]">Behavioral Profiles</h4>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {profiles.map((p) => {
                  ...
                  return (
                    <div key={p.id}
                      className="p-3 bg-[var(--bg-muted)] rounded-lg cursor-pointer hover:bg-[var(--bg-hover)] transition-colors"
                      onClick={() => setExpandedProfile(expandedProfile === p.id ? null : p.id)}>
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-1.5">
                          <Icon size={12} style={{ color }} />
                          <span className="text-[10px] font-semibold text-[var(--text-secondary)]">{p.profile_type}</span>
                        </div>
                        <span className="text-[10px] font-mono text-[var(--text-muted)]">{p.confidence}%</span>
                      </div>
                      <p className="text-[10px] text-[var(--text-muted)] mb-1.5">{p.pattern_description}</p>
                      ...
                      {expandedProfile === p.id && (
                        <div className="mt-2 pt-2 border-t border-[var(--border)] text-[10px] text-[var(--text-muted)] space-y-0.5">
