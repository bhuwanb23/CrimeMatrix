    ...
    return (
      <div className="mo-tab">
        <div className="similar-loading">
          ...
          <span>{t('Loading MO profiles...')}</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="mo-tab">
      ...
      {stats && (
        <div className="behavior-stats">
          ...
          <div className="behavior-stat">
            ...
            <span className="behavior-stat-label">{t('Comparisons')}</span>
          </div>
          <div className="behavior-stat">
            <span className="behavior-stat-value">{stats.avg_similarity || 0}%</span>
            <span className="behavior-stat-label">{t('Avg Similarity')}</span>
          </div>
        </div>
      )}
      ...
      <div className="mo-compare-form">
        <h4><Search size={14} /> {t('Compare MO Profiles')}</h4>
        <div className="mo-compare-row">
          <select className="intel-filter-select" value={selected1} onChange={(e) => setSelected1(e.target.value)}>
            <option value="">{t('Select Profile 1')}</option>
            {profiles.map((p) => (
              <option key={p.id} value={p.id}>{t('Crime')} #{p.crime_id} — {p.mo_text?.slice(0, 40) || t('N/A')}</option>
            ))}
          </select>
          ...
          <select className="intel-filter-select" value={selected2} onChange={(e) => setSelected2(e.target.value)}>
            <option value="">{t('Select Profile 2')}</option>
            {profiles.map((p) => (
              <option key={p.id} value={p.id}>{t('Crime')} #{p.crime_id} — {p.mo_text?.slice(0, 40) || t('N/A')}</option>
            ))}
          </select>
      ...
      {compareResult && (
        <div className="mo-compare-result">
          <div className="mo-compare-score">
            ...
            <span className="mo-compare-score-label">{t('Similarity')}</span>
            <span className={`mo-compare-match ${compareResult.match_level}`}>{t(compareResult.match_level)}</span>
          </div>
          {compareResult.shared_features && compareResult.shared_features.length > 0 && (
            <div className="mo-compare-features">
              <h5>{t('Shared Features')}</h5>
              <div className="mo-feature-tags">
                {compareResult.shared_features.map((f, i) => (
                  <span key={i} className="mo-feature-tag">{t(f)}</span>
                ))}
              </div>
            </div>
          )}
      ...
      {profiles.length === 0 && (
        <div className="similar-empty">
          ...
          <p>{t('No MO profiles yet')}</p>
          <span>{t('Click "Batch Fingerprint" to analyze all crimes.')}</span>
        </div>
      )}
    </div>
  )
