  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2 border-b border-[var(--border)] px-3.5 py-3">
        <h2 className="m-0 min-w-0 text-[13px] font-semibold text-[var(--text-primary)] [overflow-wrap:anywhere]">
      ...
      <div className="flex-1 overflow-y-auto">
        {selectedDistrict ? (
          <div className="border-b border-[var(--border)] p-3.5">
            <div className="mb-3 flex items-center gap-1.5 text-[11px] font-medium text-[var(--text-muted)]">
              <MapPin size={15} className="text-amber-500" aria-hidden="true" />
              <span>{t('Selected district')}</span>
            </div>
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between gap-3">
                <span className="text-xs text-[var(--text-muted)]">{t('Total cases')}</span>
                <span className="text-[13px] font-semibold text-[var(--text-primary)]">
                  {selectedDistrict.cases ?? selectedDistrict.crime_count ?? '—'}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span className="text-xs text-[var(--text-muted)]">{t('Hotspots')}</span>
                <span className="text-[13px] font-semibold text-[var(--text-primary)]">
                  {selectedDistrict.hotspots ?? '—'}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span className="text-xs text-[var(--text-muted)]">{t('Risk')}</span>
          ...
          <div className="flex flex-col items-center justify-center gap-2 border-b border-[var(--border)] px-4 py-7 text-center text-[var(--text-muted)]">
        ...
        <section className="border-b border-[var(--border)] p-3.5">
          <h3 className="mb-2.5 mt-0 flex items-center gap-1.5 text-[11px] font-semibold text-[var(--text-muted)]">
            <TrendingUp size={13} aria-hidden="true" />
            {t('Density')}
          </h3>
          <div className="flex flex-col gap-2">
            {density.length === 0 ? (
              <p className="m-0 text-xs text-[var(--text-muted)]">No density data from API</p>
            ) : density.map((d, i) => (
              <div key={i} className="flex items-center gap-2 text-xs text-[var(--text-muted)]">
                ...
                <span className="min-w-0 flex-1 text-[var(--text-primary)]">{t(d.label)}</span>
                <span className="whitespace-nowrap text-[11px] text-[var(--text-muted)]">{d.count}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="p-3.5">
          ...
          <div className="flex flex-col gap-2">
            {hotspots.length === 0 ? (
              <p className="m-0 text-xs text-[var(--text-muted)]">No hotspot data from API</p>
            ) : hotspots.map((h, i) => (
              <div key={i} className="flex items-center gap-2 text-xs text-[var(--text-muted)]">
                <span className={`size-2 shrink-0 rounded-full ${hotspotDotClass[h.severity] || 'bg-[var(--text-muted)]'}`} />
                <div className="flex min-w-0 flex-col gap-px">
                  <span className="text-[var(--text-primary)]">{h.name}</span>
                  <span className="whitespace-nowrap text-[11px] text-[var(--text-muted)]">{h.cases} {t('cases')}</span>
                </div>
              </div>
            ))}
          </div>
